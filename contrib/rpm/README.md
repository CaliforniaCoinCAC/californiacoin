...
The RPM spec file provided here is for Californiacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://californiacoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Californiacoin git repository](https://github.com/CaliforniacoinProject/Californiacoincoin) but are not part of
...
Currently only `Source10` is used, for the example `californiacoin.conf` file.
...
in the californiacoin git repository. At present only an SVG version of the Californiacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the californiacoin
...
needed when 0.12.1 is released, a proper fix is already in the Californiacoin git
master branch.
...
idea to build Californiacoin Core against a static version of that library compiled
...
The `californiacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `californiacoin-server` package. At this point, most RPM based
...
The `californiacoin-server` package should have SELinux support. How to properly do
...
Californiacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/californiacoin/). He has been
packaging Californiacoin for Fedora at least since 2012.
...
4. I renamed the `californiacoin` package that contains the Qt GUI to `californiacoin-core` as
that appears to be how the general population refers to it, in contrast to
`californiacoin-xt` or `californiacoin-classic`. I wanted to make sure the general population
